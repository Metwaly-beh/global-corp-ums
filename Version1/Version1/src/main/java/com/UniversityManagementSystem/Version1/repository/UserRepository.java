...
public interface UserRepository extends JpaRepository<User, Integer> {
  ...
  //  List<User> findByRoleRoleId(Integer roleId);
    ...
    @Query("SELECT u FROM User u WHERE u.roleName = :roleName")
    ...
    @Query("SELECT u FROM User u JOIN FETCH u.roleName WHERE u.userId = :id")
    Optional<User> findByIdWithRole(@Param("id") Integer userId);

    // Find user by username with role (for authentication)
    @Query("SELECT u FROM User u JOIN FETCH u.roleName WHERE u.username = :username")
